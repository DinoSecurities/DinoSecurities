...
- **Issuer sponsorship badges on series pages** — if the wallet behind a series is a $DINO holder (resolved via SecuritySeries → IssuerProfile → authority), the series' SecurityDetail page renders "Supported by @handle" with the issuer's tier. Pure cosmetic signal, zero storage — the resolver caches (mint → issuer wallet → tier → handle) for 10 minutes in-process.
- **Tier-gated issuer branding on the embed widget** — issuers set accent color (Bronze), logo URL (Silver), and "hide Powered by" footer (Gold) from the Issuer Portal at [/app/issue](/app/issue). Every gate is enforced server-side at write time against the live $DINO balance; past settings survive a tier drop. `/embed/:symbol` reads the issuer's branding and renders with the issuer's accent / logo / footer state — Silver-issued widgets pass for the issuer's own brand; Gold widgets look like the issuer's own product.
- **Gold-tier early marketplace access** — issuers schedule a future `publicListingAt` on a series from the Issuer Portal; non-Gold callers don't see the series in the main marketplace until the window passes. Gold holders see the full roster plus a dedicated [/app/marketplace/upcoming](/app/marketplace/upcoming) page showing every preview-scheduled listing with its public date. Discovery-only gate — the series is on-chain immediately and anyone with the mint URL can still view it directly.
- **Silver/Gold vanity mint prefixes on Create Series** — Silver-tier issuers can reserve a 2-character base58 prefix on their series's mint address; Gold unlocks 3 characters. Client-side grinding (yields to the event loop every 5000 attempts) runs before the deploy transaction; Gold prefixes take seconds to minutes depending on which three characters. Cosmetic recognition primitive, no on-chain change.
- **$DINO-weighted community governance (advisory, non-securities)** — [/app/community/governance](/app/community/governance) hosts $DINO-weighted polls on marketing, feature priorities, community grants, and community events. Gold-tier holders create proposals; Bronze-tier holders vote. **Strictly separated from on-chain securities governance:** no CPI, no signer, no treasury movement, no execute path, no reference to any security mint or issuer authority. The service layer has no `execute()` function at all — outcomes are advisory polls, acted on manually off-platform. A content lint rejects proposals containing Solana-address-shaped strings or securities-authority language (delist / halt / freeze / clawback / etc). Every page renders a prominent "Not Securities Governance" banner and creation requires an explicit disclosure acknowledgment.
