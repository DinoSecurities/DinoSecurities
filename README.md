...
- **XRPL Credentials trust layer (XLS-70d)** — on-ledger Credentials issued by trusted XRPL addresses can be accepted as an alternate KYC attestation alongside the platform's own oracle. Admin allow-list and ad-hoc verifier at `/app/issue/xrpl-credentials`; verifications are logged immutably. Every check confirms the subject has accepted the credential (`lsfAccepted`), the credential is unexpired, and the issuer is on the active trust list for the target network (mainnet / testnet / devnet). Wire-level verification only for now — `register_holder` integration lands in a follow-up once the holder-side XRPL wallet binding ships.
